   	   ...
   	   if ( $(".require_addon").exists()){
		   ...
		   var errorInput = null;	 
		    ...
		    if(errorInput) {
				...
				var $scrollTo = $(".require_addon_"+errorInput+" + p");

				// if mobile
				if($('body div#mobile-header').length) {
					$container = $("html,body");
...
$('body').on('DOMSubtreeModified', '.cart_count', function(e) {
	toggleFooterCartButton($(this).text());
});

function toggleFooterCartButton(count) {
	var cartCount = parseInt(count);
	cartCount = isNaN(cartCount) ? 0 : cartCount;
	console.log("on load: " +cartCount);

	if(cartCount > 0) {
		$('#addToCartFooterBtn').show();
		$('#addToCartFooterBtn').removeClass("d-none");
	} else {
		$('#addToCartFooterBtn').addClass("d-none");
	}
}